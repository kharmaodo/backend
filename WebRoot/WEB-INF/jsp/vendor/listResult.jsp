...
<%@include file="/WEB-INF/jsp/vendor/list.jsp"%>
...
<div id="result">
	...
	<table id="list" cellspacing="1px">
		<tr>
			<th><spring:message code="text.cloumn.email.account"/></th>
			<th width="200px"><spring:message code="text.cloumn.org"/></th>
			<th><spring:message code="text.cloumn.name"/></th>
			<th><spring:message code="text.phone"/></th>
		...
		<c:choose>
			<c:when test="${fn:length(result.objList) > 0}">
				...
				<tr>
					<td>
						<a href="<c:url value="vendor.htm?act=update&id=${dmvo.id}"/>">${dmvo.account}</a>
					</td>
					<td>
						${dmvo.vendorCategoryId}
					</td>
					<td>
						${dmvo.name}
					</td>
