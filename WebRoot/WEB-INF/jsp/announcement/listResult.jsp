...
<%@include file="/WEB-INF/jsp/announcement/list.jsp"%> 
		...
		<c:choose>
			<c:when test="${fn:length(result.objList) > 0}">
				<c:forEach var="result" items="${result.objList}" varStatus="status">
					<tr>
						<td>
							<a href="<c:out value="ann.htm?act=update&id=${result.announcementVO.id}"/>">
								<c:out value="${result.announcementVO.description}"/>
							</a>
						</td>
						<td>
							<fmt:formatDate pattern="yyyy-MM-dd" value="${result.announcementVO.publishDate}" />
						</td>
						<td>
							<fmt:formatDate pattern="yyyy-MM-dd" value="${result.announcementVO.shelveDate}" />
						</td>
						<td>
							<fmt:formatDate pattern="yyyy-MM-dd" value="${result.announcementVO.unshelveDate}" />
						</td>
						<td align="center">
							${fn:length(result.announcementDetailVOs)}
						</td>
						<td align="center">
							<c:choose>
