...
<center>
  <form name="overseasEventEditForm" method="POST" action="<c:url value="oversea.htm?act=doUpdate"/>">
  <div class="login_L">
  	<table width="800px">
  		<tr>
  			...
  			<td class="field">
  				<select name="regionCategoryId">
  					<c:forEach var="rcvo" items="${rcvos}">
  						<c:choose>
  							<c:when test="${rcvo.id == command.regionCategoryId}">
  								<option value="${rcvo.id}" selected>${rcvo.description}</option>
  							</c:when>
  							<c:otherwise>
