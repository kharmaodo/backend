...
<center>
  <form name="epaperEditForm" method="POST" enctype="multipart/form-data" action="<c:url value="epaper.htm?act=doUpdate"/>">
  <div class="login_L">
  	<table width="800px">
  		<tr>
		...
		<tr>
  			...
  			<td class="field">
  				<select name="locale">
  					<c:if test="${command.locale == 'zh_TW'}">
  						<c:set var="selectTW" value="selected"/>
  					</c:if>
  					<c:if test="${command.locale == 'zh_CN'}">
  						<c:set var="selectCN" value="selected"/>
  					</c:if>
  					<c:if test="${command.locale == 'en'}">
  						<c:set var="selectEN" value="selected"/>
  					</c:if>
  					<c:if test="${command.locale == 'ja'}">
  						<c:set var="selectJP" value="selected"/>
  					</c:if>
  					<option value="zh_TW" ${selectTW}><spring:message code="text.nameTW"/></option>
  					<option value="zh_CN" ${selectCN}><spring:message code="text.nameCN"/></option>
  					<option value="en" ${selectEN}><spring:message code="text.nameEN"/></option>
  					<option value="ja" ${selectJP}><spring:message code="text.nameJP"/></option>
  				</select>
  			</td>
  			</spring:bind>
  		</tr>
  		<tr>	
  			...
  			<td class="field"><input type="file" name="photo" class="required"/><spring:message code="epaper.current.photo"/> : <img src="${ctx}${command.photoString}" height="100"/></td>
  			</spring:bind>
  		</tr>
		<tr>	
  			<spring:bind path="command.url">
  			<td class="label"><spring:message code="epaper.url"/></td>
  			<td class="field"><input type="file" name="url" class="required"/><a href="${ctx}${command.urlString}" target="_blank"><spring:message code="epaper.current.html"/></a></td>
  			</spring:bind>
  		</tr>	
  	</table>
  	<br/>
