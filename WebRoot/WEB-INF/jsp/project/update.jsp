...
$(document).ready(function() {
	...
	<c:forEach var="task" items="${command.task}" varStatus="status">
		taskContents[${status.index}] = new nicEditor({fullPanel : true, maxHeight : 200}).panelInstance('taskContents_${status.index}');
	</c:forEach>
	<c:forEach var="resource" items="${command.resource}" varStatus="status">
		resourceContents[${status.index}] = new nicEditor({fullPanel : true, maxHeight : 200}).panelInstance('resourceContents_${status.index}');
	</c:forEach>
...
function checkSubmit() {
	...
	if(nicEditors.findEditor('first_input').getContent() == '' || 
	   nicEditors.findEditor('first_input').getContent() == '<br>' || 
	   nicEditors.findEditor('first_input').getContent() == '<BR>') {
 ...
 <table style="width:900px">
  ...
  <div class="login_L">
  	...
  	<div id="task_area">
  			<div style="position:absolute; left:3px;"><b><spring:message code="text.project.title.task"/></b></div><br>
  				...
  				<c:if test="${status.index ge 1}">
  					<c:set var="tt">id="task_${status.index}"</c:set>
  				</c:if>
  				<table ${tt} style="width:900px">
	  				<tr>
	  					<td class="label" style="width:60px"><spring:message code="text.project.description"/></td>
		      			<td class="field"><input type="text" name="taskDescriptions" id="taskDescriptions_${status.index}" value="${task.value.description}" size="50"/></td>
		      		</tr>
		      		<tr>
	      				<td class="label"><spring:message code="text.project.content"/></td>
	      				<td class="field"><textarea name="taskContents" id="taskContents_${status.index}" style="width:820px; height:200px;">${task.value.content}</textarea></td>
	      			</tr>
	      		</table>
  				</c:forEach>
  				<c:if test="${fn:length(command.task) == 0}">
  					<table id="task_0 style="width:900px">
	  				<tr>
	  					<td class="label" style="width:60px"><spring:message code="text.project.description"/></td>
  	...
  	<div id="resource_area">
  			...
  			<c:forEach var="resource" items="${command.resource}" varStatus="status">
  			<c:if test="${status.index ge 1}">
  					<c:set var="rr">id="resource_${status.index}"</c:set>
  				</c:if>
  			<table ${rr} style="width:900px">
  				<tr>
  					<td class="label" style="width:60px"><spring:message code="text.project.description"/></td>
	      			<td class="field"><input type="text" name="resourceDescriptions" id="resourceDescriptions_${status.index}" value="${resource.value.description}" size="50"/></td>
	      		</tr>
	      		<tr>
	      			<td class="label"><spring:message code="text.project.content"/></td>
	      			<td class="field"><textarea name="resourceContents" id="resourceContents_${status.index}" style="width:820px; height:200px;">${resource.value.content}</textarea></td>
	      		</tr>
	      	</table>
	      	</c:forEach>
	      	<c:if test="${fn:length(command.resource) == 0}">
	      		<table id="resource_0" style="width:900px">
  				<tr>
  					<td class="label" style="width:60px"><spring:message code="text.project.description"/></td>
